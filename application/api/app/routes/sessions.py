...
async def cancel_session(
    ...
    try:
        ...
        student_uid = session["student_uid"]  # Correct field name
        tutor_uid = session["tutor_uid"]      # Get the tutor's user ID
        
        # Check if current user is either the student or the tutor
        if current_user not in (student_uid, tutor_uid):
            raise HTTPException(
                status_code=403,
