...
async def get_current_user(
        ...
        auth_mgr: GatorGuidesAuth = Depends(get_auth_manager)
) -> int:
...
async def start_session(
    ...
    try:
        ...
        # Only student or tutor for this session can start it
        student_uid = session["student"]["uid"]
        tutor_tid = session["tutor"]["tid"]
        if current_user not in (student_uid, tutor_tid):
            raise HTTPException(
                status_code=403,
                detail="You are not allowed to start this session"
            )
...
async def end_session(
    ...
    try:
        ...
        # Only student or tutor for this session can end it
        student_uid = session["student"]["uid"]
        tutor_tid = session["tutor"]["tid"]
        if current_user not in (student_uid, tutor_tid):
            raise HTTPException(
                status_code=403,
                detail="You are not allowed to end this session"
            )
...
async def cancel_session(
    ...
    """
    Permanently delete a session. Only the student or tutor for that
    session is allowed to cancel it.
    """
    try:
        session = session_mgr.get_session(session_id)
        if not session:
        ...
        if current_user not in (student_uid, tutor_tid):
            raise HTTPException(
                status_code=403,
                detail="You are not allowed to cancel this session"
            )

        success = session_mgr.delete_session(session_id)
        if success:
            logger.info(f"Session cancelled: {session_id}")
            return {"message": "Session cancelled successfully", "sid": session_id}
        else:
            raise HTTPException(
                status_code=400,
                detail="Failed to cancel session. It may not exist."
            )

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Cancel session error: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
