...
@router.get("/tutors/{tid}/tags")
async def get_tutor_tags(
        tid: int,
        tutors_mgr: GatorGuidesTutors = Depends(get_tutors_manager)
):
    """Get all courses/tags for a specific tutor"""
    try:
        import mysql.connector

        connection = mysql.connector.connect(
            host=settings.DATABASE_HOST.split(':')[0],
            port=int(settings.DATABASE_HOST.split(':')[1]) if ':' in settings.DATABASE_HOST else 3306,
            database=settings.DATABASE_NAME,
            user=settings.DATABASE_USER,
            password=settings.DATABASE_PASSWORD
        )
        ...
        query = """
                SELECT t.tagsID AS id, t.tags AS name
                FROM TutorTags tt
                         JOIN Tags t ON tt.tagsID = t.tagsID
                WHERE tt.tid = %s \
                """
        ...
        logger.error(f"Get tutor tags error: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


# Get tutor by user ID
@router.get("/tutors/by-user/{uid}", response_model=Dict[str, Any])
async def get_tutor_by_user_id(
        uid: int,
        tutors_mgr: GatorGuidesTutors = Depends(get_tutors_manager)
):
    try:
...
async def create_rating(
    ...
    try:
        ...
        if rating:
            logger.info(
                f"Rating submitted: tid={request.tid}, uid={current_user}, rating={request.rating}"
            )
            return rating
        else:
