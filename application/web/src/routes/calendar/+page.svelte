<script lang="ts">
    ...
    // UPDATED: load both student and tutor sessions if applicable
    async function loadCalendarData() {
        ...
        try {
            ...
            // Always load sessions where the user is a STUDENT
            try {
                const studentSessionsRes = await authFetch(`/api/users/${user.uid}/sessions`);
                if (studentSessionsRes.ok) {
                    const studentSessions = await studentSessionsRes.json();
                    combined.push(
                        ...studentSessions.map((s: Session) => ({
                            ...s,
                ...
                console.warn('Failed to load student sessions for calendar:', err);
            }

            // Then check if user is also a TUTOR and load tutor sessions too
            try {
                ...
                console.warn('Failed to load tutor sessions for calendar:', err);
            }

            sessions = combined as any;
        } catch (err: any) {
